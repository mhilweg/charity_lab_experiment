...
class Task2(Page):
    ...
    def vars_for_template(player: Player):
        ...
        # Determine which responses and indices to show
        response_field = 'task2_responses' if is_task2_phase else 'task1_responses'
        responses = json.loads(getattr(player, response_field))
        starting_index = participant.vars.get('completed_rounds_in_task1', 0) if is_task2_phase else 0

        # Prepare data for the template
        return {
            'responses': {i: responses.get(i, 'NaN') for i in range(starting_index + 1, starting_index + 8)},
            'sequence_index': player.round_number - starting_index,
        }
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        is_task2_phase = participant.vars.get('in_task2_phase', False)
            ...
            # Retrieve the appropriate response dictionary
        if is_task2_phase:
            response_field = 'task2_responses'
            starting_index = participant.vars.get('completed_rounds_in_task1', 0)
        else:
            response_field = 'task1_responses'
            starting_index = 0

        # Load existing responses
        responses = json.loads(getattr(player, response_field))

        # Update responses for this round
        for i in range(1, 8):  # Iterate through the 7 responses
            response_value = getattr(player, f"response_{i}")
            responses[starting_index + i] = response_value  # Offset indices if Task 2

        # Save responses back to the player model
        setattr(player, response_field, json.dumps(responses))

        # Debugging: Log the saved responses
        print(f"Saved {response_field}: {responses}")
